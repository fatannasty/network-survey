...
function updateProgress(){
  ...
  const s=document.getElementById('stat-sections');if(s)s.textContent=(SECTIONS.indexOf(state.currentSection)+1)+'/'+SECTIONS.length;
}
...
function addFutureEq(vendor){
  ...
  div.innerHTML=`<div class="future-item-header"><span class="vendor-tag tag-${vendor}">${label}</span><button class="btn btn-danger" style="padding:4px 10px;font-size:11px" onclick="removeFuture('${fid}','${vendor}')">✕</button></div><div class="field-grid col3"><div class="field-group"><label>Model</label><input type="text" value="${model}" oninput="autoSave()"></div><div class="field-group"><label>Qty</label><input type="number" value="${qty}" min="1" oninput="autoSave()"></div><div class="field-group"><label>Location</label><input type="text" value="${loc}" oninput="autoSave()"></div></div>${specHtml}${desc?`<p class="model-desc">${desc}</p>`:''}${notes?`<p class="model-desc">Note: ${notes}</p>`:''}`;
...
function buildCableTypeBtns() {
  const wrap = document.getElementById('cable-type-btns');
  if (!wrap || typeof CABLE_TYPES === 'undefined') return;
  wrap.innerHTML = CABLE_TYPES.map((c,i) => `
    <button class="cable-type-btn ${i===0?'active':''}" id="ctype-${c.id}"
      onclick="selectCableType('${c.id}')"
...
function updateCableSummary() {
  const el = document.getElementById('mapper-cable-summary');
  if (!el || typeof mapperState === 'undefined') return;
  const n = mapperState.cables.length;
