// ── State ──
let state = {
  surveyId: '', signalLevel: 0, statusVal: '',
  futureCounts: { cisco:0, arista:0, other:0 },
  activeVendor: null, currentSection: 'site-info',
  photos: [], darkMode: false,
};
...
function togglePill(el, groupId) {
  ...
  if (!wasActive) { el.classList.add('active-'+val); state.statusVal = val; }
  else state.statusVal = '';
  autoSave();
}
...
function addEquipmentRow(data={}) {
  eqCount++;
  const id = 'eq-'+eqCount, fid = 'eqphoto-'+eqCount;
  const typeOpts = EQ_TYPES.map(t=>`<option ${t===data.type?'selected':''}>${t}</option>`).join('');
  const tr = document.createElement('tr');
  tr.id = id;
  tr.innerHTML = `
    <td><input type="text" value="${data.hostname||''}" placeholder="Hostname" oninput="autoSave()"></td>
    ...
    <td>
      <label style="cursor:pointer;font-size:18px;display:block;text-align:center;" title="Add photo">
        📷<input type="file" accept="image/*" style="display:none" onchange="attachEqPhoto(this,'${id}')">
      </label>
      <div id="${fid}" style="margin-top:4px;"></div>
    </td>
...
function attachEqPhoto(input, rowId) {
  ...
  const fid = 'eqphoto-' + rowId.replace('eq-','');
  reader.onload = e => {
    const div = document.getElementById(fid);
    if (div) div.innerHTML = `<img src="${e.target.result}" style="width:48px;height:36px;object-fit:cover;border-radius:4px;border:1px solid var(--border2);">`;
    autoSave();
  };
...
function renderPhotos() {
  ...
  state.photos.forEach((p, i) => {
    ...
    div.innerHTML = `
      ...
      <button class="photo-remove" onclick="removePhoto(${i})" aria-label="Remove photo">✕</button>`;
    grid.appendChild(div);
  });
  const addBtn = document.createElement('div');
  addBtn.className = 'photo-upload-btn';
  addBtn.innerHTML = '<span>📷</span>Add photo';
...
function addContact(data={}) {
  ...
  const div = document.createElement('div');
  div.className = 'contact-card'; div.id = id;
  div.innerHTML = `
    <button class="contact-remove" onclick="removeEl('${id}')" aria-label="Remove">✕</button>
    ...
    <div class="field-grid col2">
      <div class="field-group"><label>Email</label><input type="email" value="${data.email||''}" placeholder="name@amtrak.com" oninput="autoSave()"></div>
...
function renderVendorForm() {
  ...
  if (v==='cisco') {
    f.innerHTML = `<div class="vendor-form-inner">
      <div class="field-grid col3" style="margin-bottom:8px;">
        <div class="field-group"><label>Category</label><select id="cisco-cat" onchange="updateCiscoModels()"><option value="switches">Switches</option><option value="aps">Access points</option><option value="routers">Routers</option></select></div>
        <div class="field-group"><label>Model</label><select id="cisco-model" onchange="updateCiscoSpecs()"></select></div>
        <div class="field-group"><label>Qty</label><input type="number" id="cisco-qty" min="1" value="1"></div>
      </div>
      <div id="cisco-specs"></div><p id="cisco-desc" class="model-desc"></p>
    ...
    f.innerHTML = `<div class="vendor-form-inner">
      <div class="field-grid col3" style="margin-bottom:8px;">
        <div class="field-group"><label>Category</label><select id="arista-cat" onchange="updateAristaModels()"><option value="switches">Switches</option><option value="velo">VeloCloud SD-WAN</option><option value="aps">Access points</option></select></div>
        <div class="field-group"><label>Model</label><select id="arista-model" onchange="updateAristaSpecs()"></select></div>
        <div class="field-group"><label>Qty</label><input type="number" id="arista-qty" min="1" value="1"></div>
      </div>
      <div id="arista-specs"></div><p id="arista-desc" class="model-desc"></p>
      <div class="field-grid col2" style="margin-bottom:10px;">
    ...
    f.innerHTML = `<div class="vendor-form-inner">
      ...
      <div class="field-grid col3" style="margin-bottom:10px;">
        <div class="field-group"><label>Device type</label><select id="other-type"><option>Switch</option><option>Access point</option><option>Router</option><option>Firewall</option><option>UPS</option><option>Server</option><option>Other</option></select></div>
...
function addFutureEq(vendor) {
  ...
  if (vendor==='cisco') {
    ...
    model=document.getElementById('cisco-model').value; qty=document.getElementById('cisco-qty').value||'1';
    loc=document.getElementById('cisco-loc').value; notes=document.getElementById('cisco-notes').value;
    ...
    model=document.getElementById('arista-model').value; qty=document.getElementById('arista-qty').value||'1';
    loc=document.getElementById('arista-loc').value; notes=document.getElementById('arista-notes').value;
    label=cat==='velo'?'VeloCloud SD-WAN':cat==='aps'?'Arista AP':'Arista Switch';
    specHtml=getSpecChips('arista',cat,model); desc=getItemDesc('arista',cat,model);
  } else {
    const ov=document.getElementById('other-vendor').value||'Unknown', om=document.getElementById('other-model').value||'—';
    model=ov+' '+om; qty=document.getElementById('other-qty').value||'1';
    loc=document.getElementById('other-loc').value; notes=document.getElementById('other-notes').value;
  ...
  div.innerHTML=`
    ...
    ${notes?`<p class="model-desc">Note: ${notes}</p>`:''}`;
  document.getElementById('future-list').appendChild(div);
  autoSave();
}

function removeFuture(id, vendor) {
  const el=document.getElementById(id); if(el){el.remove(); state.futureCounts[vendor]=Math.max(0,state.futureCounts[vendor]-1); updateChips(); autoSave();}
}
...
function gatherFormData() {
  ...
  return {
    ...
    siteName:g('site-name'), surveyDate:g('survey-date'), surveyedBy:g('surveyed-by'), ticketNum:g('ticket-num'),
    siteAddress:g('site-address'), floorArea:g('floor-area'), siteType:g('site-type'),
    networkStatus:state.statusVal, signalLevel:state.signalLevel,
    ...
    futureCounts:state.futureCounts, photos: state.photos.map(p=>({name:p.name})),
...
function saveSurvey(silent=false) {
  ...
  try {
    ...
    index = index.slice(0, 50); // keep last 50
    localStorage.setItem('netsurvey_index', JSON.stringify(index));
...
function loadFromStorage() {
  ...
  try {
    ...
    set('ticket-num',d.ticketNum); set('site-address',d.siteAddress); set('floor-area',d.floorArea); set('site-type',d.siteType);
    ...
    set('building',d.building); set('room-zone',d.roomZone); set('gps',d.gps); set('landmark',d.landmark);
    set('access-instructions',d.accessInstructions); set('floorplan-notes',d.floorplanNotes);
    set('isp',d.isp); set('circuit-id',d.circuitId); set('wan-ip',d.wanIp); set('bandwidth',d.bandwidth);
    set('ssids',d.ssids); set('security-proto',d.securityProto); set('vlans',d.vlans); set('dns',d.dns);
...
function newSurvey() {
  ...
  saveSurvey(true);
  // Reset state
  state.surveyId=''; state.signalLevel=0; state.statusVal='';
  state.futureCounts={cisco:0,arista:0,other:0}; state.activeVendor=null; state.photos=[];
  ...
  document.querySelectorAll('.status-pill,.pill').forEach(p=>p.className='pill');
  ...
  document.getElementById('survey-date').value = new Date().toISOString().split('T')[0];
  addEquipmentRow(); addContact();
  goToSection('site-info');
  localStorage.setItem('netsurvey_latest','');
...
function openHistory() {
  ...
  if (!index.length) {
    list.innerHTML = '<div class="history-empty"><span>📋</span>No saved surveys yet.<br>Save your first survey to see it here.</div>';
  } else {
    list.innerHTML = index.map(s => `
      <div class="history-item" onclick="loadSurvey('${s.id}')">
        <div class="history-item-info">
          <h4>${s.name||'Untitled survey'}</h4>
          <p>${s.id} &nbsp;·&nbsp; ${s.date||'No date'} &nbsp;·&nbsp; Saved ${new Date(s.savedAt).toLocaleString()}</p>
        </div>
        <div class="history-item-actions">
          <button class="btn btn-outline-dark" style="font-size:12px;padding:6px 12px;" onclick="event.stopPropagation();deleteSurvey('${s.id}')">🗑 Delete</button>
          <button class="btn btn-primary" style="font-size:12px;padding:6px 12px;">Load →</button>
        </div>
      </div>`).join('');
  }
...
function loadSurvey(id) {
  const raw = localStorage.getItem('netsurvey_'+id);
  if (!raw) { showToast('Survey not found'); return; }
  localStorage.setItem('netsurvey_latest', id);
...
function generateQR() {
  ...
  try {
    qrInstance = new QRCode(container, {
      ...
      colorDark: '#003A5D', colorLight: '#FFFFFF',
      correctLevel: QRCode.CorrectLevel.H
    });
  } catch(e) { container.textContent = 'QR library not loaded — check internet connection.'; }
}

function regenerateQR() {
  clearTimeout(regenerateQR._t);
  regenerateQR._t = setTimeout(generateQR, 400);
}
...
function downloadQR() {
  ...
  a.download = `amtrak-netsurvey-qr.png`;
  a.click();
...
function copyQRUrl() {
  ...
  if (navigator.clipboard) navigator.clipboard.writeText(url).then(()=>showToast('URL copied ✓'));
}
...
function exportTXT() {
  ...
  const lines = [
    '╔══════════════════════════════════════╗',
    '║     AMTRAK NETWORK SITE SURVEY       ║',
    '╚══════════════════════════════════════╝',
    `Survey ID : ${d.surveyId}`,
    `Date      : ${d.surveyDate}   By: ${d.surveyedBy}`,
    `Ticket    : ${d.ticketNum||'N/A'}`,
    '',`── SITE ──`,
    `Name      : ${d.siteName}`,`Address   : ${d.siteAddress}`,
    `Floor/Area: ${d.floorArea}   Type: ${d.siteType}`,
    `Status    : ${d.networkStatus}   Signal: ${SIGNAL_LABELS[d.signalLevel]||'N/A'}`,
    '',`── LOCATION ──`,
    `Building  : ${d.building}   Room: ${d.roomZone}`,
    ...
    '',`── PLANNED EQUIPMENT ──`,
    `Cisco     : ${d.futureCounts.cisco} item(s)`,
    `Arista/VC : ${d.futureCounts.arista} item(s)`,
    `Other     : ${d.futureCounts.other} item(s)`,
    `Install   : ${d.installDate}   Priority: ${d.installPriority}`,
    `Installer : ${d.installer}   PO: ${d.poRef}`,
    `Notes     : ${d.installNotes}`,
    '',`── FINDINGS ──`,
    `Issues    : ${d.issues}`,
    `Recommend : ${d.recommendations}`,
